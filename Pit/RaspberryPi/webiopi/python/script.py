...
def process(line):
	...
	params = line.split(",")     # split csv
	count = len(params)          # count params
...
def getResponse(isPit, includeSensors, outputjson=False):
	...
	# checksum
	message += ";%s" % calc_checksum(message)
	return message
...
def sendMessage(message):
	webiopi.info("sendMessage: =%s" % str(message))
	global lastQuestion, lastAnswer
	lastQuestion = message.replace("%20", " ")
	lastAnswer = ""
